...
public class ConnectFourCLI {
    ...
    /**
     * The user manually sets up a connect four game by specifying the number of rows, columns,
     * and colors for both player tokens.
     *
    ...
    public static void printFileLines(String filename) {
        try {
            ...
            System.out.println("Missing " + filename + ". Make sure it is in the "
                + "resources folder in the project's home directory.");
        } // try
    } // printFileLines
    ...
    public static void play(ConnectFour game) {
        ...
        while (game.getPhase() != GamePhase.OVER) {
            try {
                ...
                boolean isWon = game.isLastDropConnectFour();
                if (!isWon) {
                    player = (player + 1) % 2;
                } // if
